              ...
              <div className="overflow-y-auto max-h-[60vh] pr-2 -mr-2">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredMechanics.map((mechanic) => {
                    const isUnavailable = mechanic.status === "assigned" || mechanic.status === "Busy";

                    return (
                      <div
                        key={mechanic.id}
                        onClick={() => {
                          if (isUnavailable) return; // Prevent click if unavailable
                        ...
                        } ${isUnavailable ? "pointer-events-none cursor-not-allowed" : ""}`}
                        style={isUnavailable ? { filter: "blur(2px)", opacity: 0.6 } : {}}
                      >
